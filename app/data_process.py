...
def standardize_data(data):
    ...
    for hour in repairhourslist:
        item = {
            ...
            'hours': hour.get('repairhours'),  # 维修工时
            'price': hour.get('laborhourprice'),  # 维修工时单价
            'cost': hour.get('repaircost')  # 金额
        }
    ...
    for part in carpartslist:
        item = {
            ...
            'quantity': part.get('partsquantity', 0),  # 配件数量
            'attribute': part.get('partsattribute', ''),  # 配件属性
            '_self': part.get('caruserparts'),  # 是否为自备配件  #python关键字self无法用作变量
            'price': part.get('partscost', 0),  # 单价
            'cost': part.get('partsallcost', 0),  # 金额
        }
    ...
    sum_doc = {
        'cost': data.get('total', 0),  # 总费用
        'realcost': data.get('total', 0),  # 实收总费用
    }
    ...
    std_data = {
        ...
        'deliveryMileage': data.get('repairmile', 0),  # 送修里程
        ...
        'vehicleType': carinfo.get('cartype'),  # 车辆类型 原类型：Number
        'repairType': carinfo.get('repairtype'),  # 维修类型  原类型：Number
